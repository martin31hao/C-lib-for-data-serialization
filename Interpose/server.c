...
char *unmarshalling_method(char* marshallMsg) {
	...
	func_name[idx] = '\0';
        ...
        else if (strcmp(func_name, "__xstat") == 0) { // TODO: need to be tested, may be stat
		free(func_name);
...
char *execute_write(char* msg) {
	...
	free(buf);
	//fprintf(stderr, "server write return: %s\n", add_len(ret_val));
...
char *execute_stat(char* msg) {
	...
	char *ret_val = int_to_str(stat_ret); // TODO: Need to be verified __xstat
	if (stat_ret < 0) {
		ret_val = int_to_str(-errno);
...
char *execute_getdirentries(char* msg) {
	...
	char *ret = (char *)malloc((nbytes + 20) * sizeof(char)); // TODO: Better make it on stack
